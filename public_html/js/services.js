...
export function registerCityServices({ route, go, state, api, ui, DISTRICTS, failed }) {
  ...
  route('/admin/artisans', { auth: true, tabs: '' }, async () => {
    const { items } = await api.adminArtisans();
    return `${topbar('Artisans', '/admin')}<main class="pad stack" style="gap:10px">${items.length ? items.map((a) => `<div class="card stack" style="padding:12px 14px;gap:8px;${a.hidden ? 'opacity:.6' : ''}"><div class="row" style="gap:8px"><a class="grow" href="#/artisans/${a.id}" style="font-size:14px;font-weight:650">${h(a.name)}</a>${a.verified ? '<span class="tag green">Verified</span>' : ''}${a.selfie ? '<span class="tag green">Selfie</span>' : ''}${a.hidden ? '<span class="tag">Hidden</span>' : ''}</div><div class="small muted">${h(a.trade)} · ${h(a.district)} · ${h(a.phone)} · ${a.contacts} contacts · ${rating(a.rating)} · since ${a.since}</div><div class="row" style="gap:6px"><button class="btn btn-sm ${a.verified ? 'btn-outline' : 'btn-ink'}" data-act="${a.verified ? 'unverify' : 'verify'}" data-id="${a.id}" style="width:auto">${a.verified ? 'Remove badge' : 'Verify'}</button><button class="btn btn-sm btn-outline" data-act="${a.hidden ? 'show' : 'hide'}" data-id="${a.id}" style="width:auto">${a.hidden ? 'Show' : 'Hide'}</button><a class="btn btn-sm btn-outline" href="${tel(a.phone)}" style="width:auto">Call</a></div></div>`).join('') : '<div class="small muted">Nobody has listed yet.</div>'}<div class="small muted" style="line-height:1.5">Verify only after calling them: does the number ring, is the trade real, is the district right. The badge shows on their card.</div></main>`;
  }, { mount(el) { el.querySelectorAll('[data-act]').forEach((b) => b.addEventListener('click', async () => { try { await api.adminArtisan(b.dataset.id, b.dataset.act); location.reload(); } catch (err) { failed(el, err); } })); } });
