...
export function registerCityServices({ route, go, state, api, ui, DISTRICTS, failed }) {
  ...
  route('/meetup/:id', { auth: true, tabs: '' }, async ({ id }) => {
    ...
    mount(el, { id }) {
      ...
      el.querySelector('#cancel')?.addEventListener('click', () => { if (confirm('Cancel this event? Everyone registered will be told.')) act(() => api.cancelEvent(id), 'Cancelled'); });
  ...
  function artisanCard(a) {
    return `<a class="card row" href="#/artisans/${a.id}" style="padding:12px;gap:12px;align-items:flex-start">
      ...
      <div class="grow" style="min-width:0">
        <div class="row" style="gap:6px"><span style="font-size:15px;font-weight:700">${h(a.name)}</span>${a.verified ? icon('circle-check') : ''}</div>
