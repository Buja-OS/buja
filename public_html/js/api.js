...
export const api = {
  ...
  claimSpot:       (id, b) => request('POST', '/spots/' + id + '/claim', b),
  suggestSpot:     (id, field, value) => request('POST', '/spots/' + id + '/suggest', { field, value }),
  ownerEditSpot:   (id, b) => request('PATCH', '/spots/' + id + '/owner', b),
  ...
  setBusy:         (minutes) => request('PATCH', '/artisans/me/busy', { minutes }),
  ...
  escrowBanks:     () => request('GET', '/escrow/banks'),
  escrowAccount:   () => request('GET', '/escrow/account'),
  escrowAccountCheck: (bank, number) => request('GET', '/escrow/account?bank=' + encodeURIComponent(bank) + '&number=' + encodeURIComponent(number)),
  ...
  adminOrderPayAct:(id, action) => request('POST', '/admin/order-pay/' + id + '/' + action),
  adminClaims:     () => request('GET', '/admin/claims'),
  decideClaim:     (id, decision, reason) => request('POST', '/admin/claims/' + id, { decision, reason }),
  decideEdit:      (id, decision) => request('POST', '/admin/spot-edits/' + id, { decision }),
  kartAchievements: () => request('GET', '/kart/achievements'),
  kartFriendGhosts: (f) => request('GET', '/kart/friend-ghosts' + qs(f)),
  kartPerf:        (b) => request('POST', '/kart/perf', b),
  kartTierHint:    (gpu) => request('GET', '/kart/tier-hint?gpu=' + encodeURIComponent(gpu)),
  adminKartPerf:   () => request('GET', '/admin/kart-perf'),
  ...
  kartRoomTrack:   (code, track) => request('POST', '/kart/rooms/' + code + '/track', { track }),
  kartFriends:     () => request('GET', '/kart/friends'),
  ...
  importSpots:     (category, tile) => request('POST', '/admin/spots-import', { category, tile }),
  spotsQuery:      (category, tile) => request('GET', '/admin/spots-query' + qs({ category, tile })),
  spotsIngest:     (category, elements, source) => request('POST', '/admin/spots-ingest', { category, elements, source }),
  ...
  fuelImport:      (tile) => request('POST', '/fuel/import', { tile }),
