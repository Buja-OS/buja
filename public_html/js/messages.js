  ...
  function bubble(m) {
    ...
    if (m.type === 'call') {
      ...
      return `<div class="card stack" style="align-self:${m.mine ? 'flex-end' : 'flex-start'};max-width:280px;padding:14px;gap:10px;border-color:var(--orange)">
        ...
        ${soon ? `<a class="btn btn-sm btn-primary" href="#/rtc/${h(meta.room)}">${icon(meta.mode === 'audio' ? 'phone' : 'video')} Join the call</a>` : `<div class="small muted">Opens 15 minutes before</div>`}
