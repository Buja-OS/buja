...
export function registerTrust({ route, go, state, setState, api, ui, failed }) {
  ...
  route('/admin/spots', { auth: true, tabs: 'Me' }, async () => {
    const g = guard(); if (g) return g; const [{ items }, cq] = await Promise.all([api.adminSpots(), api.adminClaims().catch(() => ({ claims: [], edits: [] }))]);
    const FIELD = { hours: 'Opening hours', phone: 'Phone number', closed: 'Says it has closed' };
    const claimsHtml = (cq.claims.length || cq.edits.length) ? `<div class="pad stack" style="gap:12px;padding-bottom:0">
      ${cq.claims.length ? `<div class="section" style="margin:0">BUSINESS CLAIMS · ${cq.claims.length}</div>${cq.claims.map((c) => `<div class="card stack" style="padding:14px;gap:8px" data-claim="${c.id}">
        <div class="row" style="gap:8px"><div class="grow"><div class="h-sm">${h(c.spot ? c.spot.name : 'Place')}</div><div class="small muted">${c.spot ? h(c.spot.category) + ' · ' + h(c.spot.district) : ''}${c.spot && c.spot.phone ? ' · listed phone ' + h(c.spot.phone) : ''}</div></div>${c.spot ? `<a class="btn btn-sm btn-outline" href="#/ask/place/${c.spot.id}" style="width:auto">Open</a>` : ''}</div>
        <div class="small" style="line-height:1.5"><strong>${h(c.user ? c.user.name : '?')}</strong> says they are the <strong>${h(c.role)}</strong>. Business phone: <a href="tel:${h(c.phone)}"><strong>${h(c.phone)}</strong></a>${c.user && c.user.phone ? ' · account phone ' + h(c.user.phone) : ''}${c.business ? ' · on Buja as ' + h(c.business) : ''}</div>
        ${c.note ? `<div class="small muted">"${h(c.note)}"</div>` : ''}
        ${c.proof ? `<img src="${h(c.proof)}" alt="Proof" data-zoom="${h(c.proof)}" style="width:100%;max-height:220px;object-fit:cover;border-radius:12px">` : '<div class="small muted">No proof photo. Call the business phone to check.</div>'}
        <input class="input" data-why placeholder="Reason if you reject (they will see it)">
        <div class="row" style="gap:8px"><button class="btn btn-sm btn-primary grow" data-cdec="approve">${icon('circle-check')} Approve</button><button class="btn btn-sm btn-outline grow" data-cdec="reject">Reject</button></div></div>`).join('')}` : ''}
      ${cq.edits.length ? `<div class="section" style="margin:0">CORRECTIONS FROM RESIDENTS · ${cq.edits.length}</div><div class="card list">${cq.edits.map((e) => `<div class="item" data-edit="${e.id}" style="flex-wrap:wrap;gap:8px"><div class="grow" style="min-width:200px"><div class="t">${h(e.spot ? e.spot.name : 'Place')} · ${h(FIELD[e.field] || e.field)}</div><div class="s">${e.field === 'closed' ? 'Reported closed' + (e.value ? ': ' + h(e.value) : '') : '<strong>' + h(e.value || '') + '</strong>' + (e.spot && e.spot.now ? ' (now: ' + h(e.spot.now) + ')' : '')} · by ${h(e.by)}</div></div><button class="btn btn-sm btn-primary" data-edec="approve" style="width:auto">Apply</button><button class="btn btn-sm btn-outline" data-edec="reject" style="width:auto">Ignore</button></div>`).join('')}</div>` : ''}</div>` : '';
    return `${topbar('Places to verify', '/admin')}${claimsHtml}<div class="pad" style="padding-bottom:0"><div class="card stack" style="padding:14px;gap:10px"><div class="h-sm">Fill the directory from OpenStreetMap</div><div class="small muted" style="line-height:1.5">Pulls every named place of a category across the FCT. Real places, real coordinates, marked as unreviewed until somebody rates them. Run each once; running again only adds what is new.</div><button class="btn btn-primary" id="importall">Import everything</button><div class="small muted" style="margin-top:-4px">All 11 kinds of place, in four parts of the FCT each. Your phone fetches the map data itself (about 10 to 20 MB of data), so keep this screen open; it usually takes 5 to 15 minutes. Or do one kind:</div><div class="row" style="gap:6px;flex-wrap:wrap">${['food', 'lounge', 'worship', 'health', 'hotel', 'shopping', 'relax', 'nightlife', 'culture', 'kids', 'services'].map((c) => `<button class="btn btn-sm btn-outline" data-import="${c}" style="width:auto">${c}</button>`).join('')}</div><div class="small" id="importres" style="line-height:1.5;white-space:pre-line"></div><div class="row" style="gap:8px;margin-top:4px"><button class="btn btn-sm btn-ink" id="fuelimport" style="width:auto">Import fuel stations</button><span class="small muted">for the Fuel board</span></div></div>
    ...
    <main class="pad stack" style="gap:12px">${items.length ? items.map((i) => `<div class="card stack" style="padding:14px;gap:8px" data-s="${i.id}"><div class="h-sm">${h(i.name)}</div><div class="small muted">${i.category} · ${h(i.district)}${i.area ? ' · ' + h(i.area) : ''} · added by ${h(i.addedBy || 'Buja')} · ${i.createdAt.slice(0, 10)}</div><div class="small" style="line-height:1.5">${h(i.description)}</div>${i.tags.length ? `<div class="row" style="flex-wrap:wrap;gap:6px">${i.tags.map((t) => `<span class="tag" style="background:var(--surface);color:var(--ink-2)">${h(t)}</span>`).join('')}</div>` : ''}<div class="row" style="gap:8px"><button class="btn btn-sm btn-ink" data-act="verify" style="flex:1">${icon('circle-check')} Verify</button><button class="btn btn-sm btn-outline" data-act="remove" style="flex:1">Remove</button></div></div>`).join('') : `<div class="placeholder" style="padding:50px 0"><div class="h-md">Nothing to verify</div></div>`}</main>`;
      ...
      el.querySelectorAll('[data-cdec]').forEach((b) => b.addEventListener('click', async () => { const card = b.closest('[data-claim]'); if (b.dataset.cdec === 'approve' && !confirm('Approve this claim? The place becomes theirs to manage.')) return; busy(b, true); try { await api.decideClaim(card.dataset.claim, b.dataset.cdec, card.querySelector('[data-why]').value); card.remove(); toast(b.dataset.cdec === 'approve' ? 'Approved. They have been told.' : 'Rejected'); } catch (err) { busy(b, false); failed(el, err); } }));
      el.querySelectorAll('[data-edec]').forEach((b) => b.addEventListener('click', async () => { const row = b.closest('[data-edit]'); busy(b, true); try { await api.decideEdit(row.dataset.edit, b.dataset.edec); row.remove(); toast(b.dataset.edec === 'approve' ? 'Applied' : 'Ignored'); } catch (err) { busy(b, false); failed(el, err); } }));
