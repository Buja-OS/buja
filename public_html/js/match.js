...
export function registerMatch({ route, go, state, api, ui, DISTRICTS, failed }) {
  ...
  route('/match/setup', { auth: true, tabs: '' }, async () => {
    ...
    return `${topbar('Who you see', '/match/setup?step=3')}${dots}
      <form id="f" class="pad stack" style="gap:16px;padding-top:8px" novalidate>
        ...
        <div class="card stack" style="padding:16px;gap:12px">
          ...
          <div class="field"><label for="radiusKm">Or set a distance in kilometres</label><select class="input" id="radiusKm"><option value="">No distance limit</option>${[2, 5, 10, 15, 25, 40].map((n) => `<option value="${n}" ${p?.radiusKm === n ? 'selected' : ''}>Within ${n} km of me</option>`).join('')}</select><div class="hint">${p?.hasLocation ? 'Using the location you shared. Distances show on every card.' : 'Needs your location. Tap the button below once and Buja works out real distances.'}</div></div>
          ${p?.hasLocation ? `<button type="button" class="btn btn-sm btn-outline" id="refreshloc">${icon('location-dot')} Update my location</button>` : `<button type="button" class="btn btn-sm btn-ink" id="refreshloc">${icon('location-dot')} Use my location</button>`}
          <div class="small muted" style="line-height:1.5">Your exact spot is never shown. Buja stores it roughly, to about 100 metres, and only ever shows other people a distance.</div>
        </div>
    ...
    mount(el) {
      ...
      el.querySelector('#refreshloc')?.addEventListener('click', (e) => { const b = e.currentTarget; if (!navigator.geolocation) { toast('This phone cannot share location.'); return; } busy(b, true); navigator.geolocation.getCurrentPosition(async (p) => { try { await api.setLocation(p.coords.latitude, p.coords.longitude); toast('Location saved. Distances are on.'); busy(b, false); } catch (err) { busy(b, false); failed(el, err); } }, () => { busy(b, false); toast('Could not get your location. Check the permission for this site.'); }, { enableHighAccuracy: true, timeout: 12000 }); });
      ...
      el.querySelector('#f')?.addEventListener('submit', async (e) => {
        ...
        if (step === '4') body = { ageMin: +el.querySelector('#ageMin').value, ageMax: +el.querySelector('#ageMax').value, nearbyOnly: segVal('nearby') === '1', radiusKm: el.querySelector('#radiusKm').value || null, visible: sw.classList.contains('on') };
  ...
  function card(c) {
    return `<div class="card" data-card="${c.id}" style="position:relative;flex:1;border-radius:24px;overflow:hidden;display:flex;flex-direction:column;justify-content:flex-end;min-height:420px">
      ...
      <div style="position:relative;padding:60px 20px 20px;background:linear-gradient(180deg,rgba(0,0,0,0) 0%,rgba(0,0,0,.78) 100%);color:#fff;display:flex;flex-direction:column;gap:6px">
        ...
        <div style="font-size:14px;color:#D0D0D6" class="row">${icon('location-dot')} ${c.km != null ? `<strong style="color:#fff">${c.km <= 2 ? 'under 2 km away' : 'about ' + c.km + ' km away'}</strong> · ` : ''}${h(c.district)}${c.work ? ' · ' + h(c.work) : ''}</div>
  ...
  route('/match/profile/:id', { auth: true, tabs: '' }, async ({ id }) => {
    ...
    <main class="pad stack" style="gap:14px;padding-top:18px">
      <div><div class="row" style="gap:8px"><span class="h-lg" style="font-size:26px">${h(p.name)}, ${p.age}</span>${p.verified ? `${icon('circle-check')}<span class="small" style="font-weight:700;color:var(--green-dark)">VERIFIED</span>` : p.phoneVerified ? `<span class="small muted" style="font-weight:600">PHONE VERIFIED</span>` : ''}</div><div class="muted small row" style="gap:6px;margin-top:4px">${icon('location-dot')} ${p.km != null ? `<strong style="color:var(--ink)">${p.km < 1 ? 'under 1 km away' : p.km + ' km away'}</strong> · ` : ''}${h(p.district)} · ${prox(p.match.proximity)}${p.likedYou ? ' · <strong style="color:var(--orange-dark)">Liked you</strong>' : ''}</div></div>
    ...
    mount(el, { id }) {
      ...
      el.querySelector('#more').addEventListener('click', () => { el.querySelector('#menu').innerHTML = `<div class="card list"><button class="item" data-block><div class="mi">${icon('eye-slash')}</div><div class="grow"><div class="t">Block</div><div class="s">They will not see you and you will not see them</div></div></button><button class="item" data-report><div class="mi">${icon('triangle-exclamation')}</div><div class="grow"><div class="t">Report</div><div class="s">Fake profile, harassment, scam</div></div></button></div>`;
        ...
        el.querySelector('[data-report]').addEventListener('click', () => go('/report/user/' + id)); });
