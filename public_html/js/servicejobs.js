...
export function registerJobs({ route, go, state, api, ui, failed }) {
  ...
  const NOUN = { mechanic: ['mechanic', 'mechanics'], vulcanizer: ['tyre man', 'tyre men'], towing: ['tow truck', 'tow trucks'], electrician: ['electrician', 'electricians'], plumber: ['plumber', 'plumbers'], hair: ['barber', 'barbers'], tailor: ['tailor', 'tailors'], ac: ['AC repairer', 'AC repairers'], locksmith: ['locksmith', 'locksmiths'], restaurant: ['place to eat', 'places to eat'], suya: ['suya spot', 'suya spots'], bakery: ['bakery', 'bakeries'], drinks: ['drinks seller', 'drinks sellers'], grocery: ['shop', 'shops'], water: ['water seller', 'water sellers'], gas: ['gas seller', 'gas sellers'], cook: ['cook', 'cooks'], laundry: ['laundry', 'laundries'], '': ['artisan', 'artisans'] };
  const driveMin = (km) => km == null ? null : Math.max(2, Math.round(km * 1.35 / 24 * 60 + 2));   // Abuja town driving, with the road longer than the straight line
  ...
  const ORDER_TRADES = { cook: ['What would you like?', '2 plates of jollof rice with chicken, 1 pack of moi moi'], laundry: ['What needs washing?', '8 shirts, 3 trousers, 1 duvet. Pick up and return ironed'], errand: ['What should they get or deliver?', 'Buy 2 bags of pure water and a gas refill from Wuse market'],
    restaurant: ['What would you like?', '1 plate of jollof rice with chicken, 1 bottle of water'], suya: ['What would you like?', '₦3,000 beef suya with extra pepper, 2 sticks of chicken'], bakery: ['What would you like?', 'A 2-layer chocolate cake for Saturday, with "Happy birthday Ada"'],
    drinks: ['What would you like?', '6 bottles of malt, 1 pack of zobo'], grocery: ['What should they bring?', '1 bag of rice (25 kg), 1 carton of noodles, 2 tins of tomato'], water: ['How much water?', '10 bags of pure water, 2 dispenser bottles'], gas: ['What cylinder size?', 'Refill my 12.5 kg cylinder. I will have it ready at the gate'] };
  const isOrder = (j) => j && j.kind === 'order';
  const QUICK = [['mechanic', 'Mechanic'], ['vulcanizer', 'Tyres'], ['towing', 'Towing'], ['electrician', 'Electrician'], ['plumber', 'Plumber'], ['hair', 'Barber & hair'], ['tailor', 'Tailor'], ['ac', 'AC repair'], ['locksmith', 'Locksmith'], ['restaurant', 'Food'], ['suya', 'Suya'], ['grocery', 'Groceries'], ['water', 'Water'], ['gas', 'Gas'], ['', 'All']];
    ...
    async mount(el) {
      ...
      const detail = (a) => {
        ...
        sheet.body.innerHTML = `<div class="stack" style="gap:12px;padding-top:4px">
          ...
          <div class="row small" style="gap:6px;flex-wrap:wrap">${a.onDuty ? '<span class="tag green">Open now</span>' : '<span class="tag" style="background:var(--surface)">Closed now</span>'}${a.online ? '<span class="tag" style="background:var(--green-tint);color:var(--green-dark)">● Active on Buja now</span>' : ''}${a.km != null ? `<span class="tag" style="background:var(--surface)">About ${driveMin(a.km)} min drive</span>` : ''}</div>
          ${a.orderable ? `<a class="btn btn-primary" href="#/artisans/${a.id}?menu=1">${icon('bag-shopping')} See menu and order</a>` : `<button class="btn btn-primary" id="askcome" ${a.available ? '' : 'disabled'}>${icon('location-dot')} ${a.available ? 'Ask them to come to me' : 'Not taking jobs right now'}</button>`}
          ${a.lat != null ? `<a class="btn btn-outline" href="${h(ui.placeHref({ lat: a.lat, lng: a.lng, name: a.name, sub: [a.tradeLabel, a.district].filter(Boolean).join(' · '), icon: TRADE_ICON[a.trade] || 'wrench', color: TRADE_COLOR[a.trade] || '#FF7A1A', photo: a.photo || '' }))}">${icon('diamond-turn-right')} Find the way to them</a>` : ''}
      ...
      const renderList = () => {
        ...
        const again = mine.length ? `<div style="font-size:15px;font-weight:800;margin:0 0 8px">Your ${trade === 'towing' ? 'tow trucks' : trade === 'vulcanizer' ? 'tyre men' : !trade || trade === 'mechanic' ? 'mechanics' : 'favourites'}</div><div class="row" style="gap:10px;overflow-x:auto;padding-bottom:12px;margin-bottom:6px">${mine.map((m) => `<div class="card stack" style="padding:10px;gap:6px;min-width:128px;align-items:center;text-align:center;flex-shrink:0">${m.photo ? `<img src="${h(m.photo)}" alt="" style="width:52px;height:52px;border-radius:26px;object-fit:cover">` : `<span class="lst-face" style="--c:#FF7A1A"><b>${h(m.name.slice(0, 2).toUpperCase())}</b></span>`}<div style="font-weight:700;font-size:13px;line-height:1.2">${h(m.name)}</div><div class="small muted">${m.jobs ? m.jobs + ' job' + (m.jobs > 1 ? 's' : '') + ' for you' : 'Saved'}${m.available && m.onDuty ? ' · online' : ''}</div><a class="btn btn-sm btn-primary" href="#/breakdown?trade=${h(m.trade)}&again=${m.id}&name=${encodeURIComponent(m.name)}" style="width:100%">Call again</a></div>`).join('')}</div>` : '';
        sheet.body.innerHTML = '<i data-sheet-list hidden></i>' + sos + again + `<div class="row" style="padding:2px 0 10px"><div class="grow"><div style="font-size:17px;font-weight:800">${list.length ? list.length + ' ' + noun(trade, list.length) + ' near you' : 'Nobody here yet'}</div><div class="small muted">${me ? (sort === 'rating' ? 'Best rated first.' : sort === 'open' ? 'Open now first, then closest.' : 'Closest first.') + ' Tap one on the map or below.' : 'Turn on location to see who is closest.'}</div></div></div>
          ${list.length > 1 ? `<div class="row" style="gap:6px;margin:-2px 0 10px">${[['', 'Nearest'], ['open', 'Open now'], ['rating', 'Top rated']].map(([k, l]) => `<button class="chip ${sort === k ? 'on' : ''}" data-sort="${k}">${l}</button>`).join('')}</div>` : ''}
          ${list.length ? list.map((a) => `<button class="row card" data-a="${a.id}" style="width:100%;text-align:left;padding:10px 12px;gap:12px;margin-bottom:8px;background:var(--card)"><span class="lst-face" style="--c:${TRADE_COLOR[a.trade] || '#FF7A1A'}">${a.photo ? `<img src="${h(a.photo)}" alt="" loading="lazy" onerror="this.remove()">` : ''}<b>${h((a.name || '?').trim().split(/\s+/).slice(0, 2).map((w) => w[0]).join('').toUpperCase())}</b><i>${icon(TRADE_ICON[a.trade] || 'wrench')}</i></span><span class="grow" style="min-width:0"><span style="display:block;font-size:14px;font-weight:700;white-space:nowrap;overflow:hidden;text-overflow:ellipsis">${h(a.name)}</span><span class="small muted" style="display:block">${h(a.tradeLabel)}${a.km != null ? ' · ' + a.km + ' km · about ' + driveMin(a.km) + ' min drive' : ''}</span><span class="row" style="gap:6px;margin-top:3px;flex-wrap:wrap">${a.onDuty ? '<span class="tag green" style="font-size:11px">Open now</span>' : '<span class="tag" style="font-size:11px;background:var(--surface)">Closed now</span>'}${a.online ? '<span class="tag" style="font-size:11px;background:var(--green-tint);color:var(--green-dark)">● Active now</span>' : ''}${a.orderable ? '<span class="tag" style="font-size:11px;background:var(--orange-tint);color:var(--orange-dark)">Order in app</span>' : a.mobileService ? '<span class="tag" style="font-size:11px;background:var(--surface)">Comes to you</span>' : ''}${a.emergency ? '<span class="tag" style="font-size:11px;background:var(--surface)">24 hours</span>' : ''}</span></span><span class="small" style="font-weight:700;color:#B7791F">${starsShort(a.rating)}</span></button>`).join('') : `<div class="small muted" style="line-height:1.5">No ${trade ? 'one in this trade' : 'artisans'} listed near you yet. Know a good one? Ask them to list themselves under Artisans.</div><a class="btn btn-outline" href="#/artisans/me" style="margin-top:10px">List my own trade</a>`}`;
        sheet.body.querySelectorAll('[data-a]').forEach((b) => b.addEventListener('click', () => detail(list.find((x) => String(x.id) === b.dataset.a))));
        sheet.body.querySelectorAll('[data-sort]').forEach((b) => b.addEventListener('click', () => { sort = b.dataset.sort; load(); }));
      };
      ...
      const load = async () => {
        ...
        try { const r = await api.artisans({ trade, lat: me?.lat || '', lng: me?.lng || '', sort }); list = (r.artisans || []).filter((a) => a.lat != null || !map); } catch (err) { failed(el, err); return; }
        if (map) {
  ...
  route('/artisans/register', { auth: true, tabs: '' }, async () => {
    ...
    <form id="af" class="pad stack" style="gap:14px" novalidate>
      ...
      <section data-step="2" class="stack" style="gap:14px;display:none">
        <div><div class="h-md">Where you work</div><div class="small muted" id="s2note">Step 2 of 3. Put the pin exactly on your workshop. Breakdowns near it will reach you first.</div></div>
        <div id="pinmap" style="height:260px;border-radius:16px;overflow:hidden;position:relative"></div>
        <div class="row" style="gap:8px"><button type="button" class="btn btn-sm btn-outline grow" id="gps">${icon('location-crosshairs')} <span id="gpstxt">I am at my workshop now</span></button></div>
        ...
        <div class="row" style="gap:10px"><div class="field grow" style="margin:0"><label for="radiusKm" id="radlbl">How far you travel</label><select class="input" id="radiusKm">${[5, 10, 15, 20, 30, 45].map((n) => `<option value="${n}" ${(me ? me.radiusKm : 15) === n ? 'selected' : ''}>${n} km</option>`).join('')}</select></div>${field({ id: 'hours', label: 'Working hours', value: (me && me.hours) || '', placeholder: 'Mon to Sat, 7am to 7pm' })}</div>
        <label class="check" style="align-items:center"><input type="checkbox" id="mobileService" ${!me || me.mobileService ? 'checked' : ''}><span id="mobtxt">I come to the customer (roadside and home repairs)</span></label>
        <label class="check" style="align-items:center"><input type="checkbox" id="emergency" ${me && me.emergency ? 'checked' : ''}><span id="emtxt">I take night and weekend emergencies</span></label>
      ...
      <section data-step="3" class="stack" style="gap:14px;display:none">
        ...
        <div class="row" style="gap:14px;align-items:center"><div id="facebox" style="width:88px;height:88px;border-radius:44px;background:var(--surface);overflow:hidden;display:flex;align-items:center;justify-content:center;flex-shrink:0">${me && me.photo ? `<img src="${h(me.photo)}" alt="" style="width:100%;height:100%;object-fit:cover">` : icon('user')}</div>
          <div class="stack" style="gap:6px"><label class="btn btn-sm btn-primary" style="width:auto;cursor:pointer">${icon('camera')} <span id="facetxt">${me && me.photo ? 'Change photo' : 'Add a photo of your face'}</span><input type="file" accept="image/*" capture="user" id="face" style="display:none"></label><div class="small muted">Clear, facing the camera, no sunglasses.</div></div></div>
    ...
    async mount(el) {
      ...
      const adapt = () => {
        const t = el.querySelector('#trade').value, biz = !!ORDER_TRADES[t], car = ['mechanic', 'vulcanizer', 'towing'].includes(t);
        const set = (sel, a, b) => { const n = el.querySelector(sel); if (n) n.textContent = biz ? a : b; };
        el.querySelector('#svcbox').style.display = biz || !(SERVICES[t] || []).length ? 'none' : '';
        const bb = el.querySelector('#brandsbox'); if (bb) bb.style.display = car ? '' : 'none';
        const cf = el.querySelector('#calloutFee')?.closest('.field'); if (cf) cf.style.display = biz ? 'none' : '';
        el.querySelector('#business').placeholder = biz ? (t === 'suya' ? 'Mallam Musa Suya Spot' : t === 'grocery' ? 'Mama Ngozi Provisions' : t === 'water' ? 'Crystal Table Water' : t === 'gas' ? 'Wuse Gas Refill' : 'Mama Chidi\'s Kitchen') : 'Musa Auto Clinic';
        const bl = el.querySelector('label[for="business"]'); if (bl) bl.textContent = biz ? 'Business name' : 'Workshop or business name (optional)';
        el.querySelector('#about').placeholder = biz ? 'What you sell and when you open, e.g. Home-cooked jollof, soups and swallow. Open 10am to 9pm. We deliver within Wuse and Maitama.' : 'Toyota and Honda specialist, 12 years, genuine parts, I explain the fault before I fix it.';
        const al = el.querySelector('label[for="about"]'); if (al) al.textContent = biz ? 'About your business' : 'About your work (optional)';
        set('#s2note', 'Step 2 of 3. Put the pin exactly on your shop or kitchen. Orders are delivered from here and nearby customers find you first.', 'Step 2 of 3. Put the pin exactly on your workshop. Breakdowns near it will reach you first.');
        set('#gpstxt', 'I am at my shop now', 'I am at my workshop now');
        set('#radlbl', 'How far you deliver', 'How far you travel');
        set('#mobtxt', 'I deliver to customers', 'I come to the customer (roadside and home repairs)');
        set('#emtxt', 'I open late at night', 'I take night and weekend emergencies');
        if (!(me && me.photo)) set('#facetxt', 'Add a photo of you or your shop', 'Add a photo of your face');
      };
      el.querySelector('#trade').addEventListener('change', () => { drawSvc(); adapt(); }); drawSvc(); adapt();
      ...
      el.querySelector('#af').addEventListener('submit', async (e) => {
        ...
        try {
          ...
          const biz = !!ORDER_TRADES[el.querySelector('#trade').value];
          toast(me ? 'Saved' : biz ? 'You are registered. Now add your menu so customers can order.' : 'You are registered. Keep notifications on so you never miss a breakdown.');
          go(biz && !me ? '/artisans/menu' : '/artisans/dashboard');
        } catch (err) {
          busy(btn, false);
  ...
  route('/artisans/dashboard', { auth: true, tabs: 'Me' }, async () => {
    ...
    const title = a.orderable ? 'My business' : a.tradeKey === 'mechanic' ? 'Mechanic dashboard' : 'My work';
    return `${topbar(title, '/me', `<a class="iconbtn" href="#/artisans/register" aria-label="Edit my profile">${icon('user')}</a>`)}
    <main class="pad stack" style="gap:14px">
      ...
      <div class="card stack" style="padding:14px;gap:10px"><div class="h-sm">How you are doing</div>
        ...
        <div class="small muted" style="line-height:1.5">${a.orderable ? 'Buja shows nearby, quick, well-rated businesses first, and moves ones who drop orders down.' : 'Buja alerts the nearest mechanics first, then moves quick, well-rated mechanics up and ones who drop jobs down.'}</div></div>
  ...
  /* ============================== MY MENU (restaurants, suya, bakeries, drinks, groceries, water, gas…) ============================== */
  route('/artisans/menu', { auth: true, tabs: '' }, async () => {
    let d; try { d = await api.myMenu(); } catch (err) { return `${topbar('Your menu', '/artisans/dashboard')}<div class="placeholder" style="padding:60px 20px"><div class="mi card">${icon('receipt')}</div><div class="h-md">No menu for this trade</div><div class="small muted">${h(err.message || 'Register your business first.')}</div><a class="btn btn-primary" href="#/artisans/register" style="width:auto">Open my business profile</a></div>`; }
    const N = (n) => '₦' + Number(n || 0).toLocaleString();
    const secs = [...new Set(d.menu.map((m) => m.section).filter(Boolean))];
    return `${topbar('Your menu', '/artisans/dashboard')}
    <main class="pad stack" style="gap:14px">
      <div class="card stack" style="padding:14px;gap:10px"><div class="h-sm">Delivery</div>
        <div class="row" style="gap:10px"><div class="field grow" style="margin:0"><label for="dfee">Delivery fee (₦)</label><input class="input" id="dfee" type="number" inputmode="numeric" min="0" placeholder="0 = included" value="${d.deliveryFee ?? ''}"><div class="error" data-error="deliveryFee"></div></div>
          <div class="field grow" style="margin:0"><label for="dmin">Smallest order (₦)</label><input class="input" id="dmin" type="number" inputmode="numeric" min="0" placeholder="None" value="${d.minOrder ?? ''}"></div></div>
        <button class="btn btn-sm btn-ink" id="dsave">Save delivery</button></div>
      <div class="row" style="align-items:flex-end"><div class="grow"><div class="section" style="margin:0">ITEMS</div><div class="small muted">${d.menu.length ? d.menu.length + ' on your menu. Switch one off when it finishes.' : 'Nothing yet. Add your first dish or item below.'}</div></div></div>
      <div class="stack" style="gap:8px" id="items">${d.menu.slice().sort((x, y) => (secs.indexOf(x.section) - secs.indexOf(y.section)) || 0).map((m) => `<div class="card mn-row ${m.available ? '' : 'mn-off'}" style="padding:10px 12px" data-row="${m.id}">
        ${m.photo ? `<img src="${h(m.photo)}" alt="" style="width:52px;height:52px;border-radius:10px;object-fit:cover;flex-shrink:0">` : `<span style="width:52px;height:52px;border-radius:10px;background:var(--surface);color:var(--ink-3);display:flex;align-items:center;justify-content:center;flex-shrink:0">${icon('image')}</span>`}
        <div class="grow" style="min-width:0"><div style="font-weight:700">${h(m.name)}</div><div class="small muted">${m.section ? h(m.section) + ' · ' : ''}${N(m.price)}${m.available ? '' : ' · finished'}</div></div>
        <button class="switch ${m.available ? 'on' : ''}" data-avail="${m.id}" role="switch" aria-checked="${m.available}" aria-label="Available" style="flex-shrink:0"><span></span></button>
        <button class="iconbtn" data-edit="${m.id}" aria-label="Edit">${icon('pen')}</button></div>`).join('')}</div>
      <form class="card stack" id="mf" style="padding:14px;gap:10px"><div class="h-sm" id="mftitle">Add an item</div>
        <div class="field" style="margin:0"><label for="name">Name</label><input class="input" id="name" maxlength="80" placeholder="Jollof rice and chicken"><div class="error" data-error="name"></div></div>
        <div class="row" style="gap:10px"><div class="field grow" style="margin:0"><label for="price">Price (₦)</label><input class="input" id="price" type="number" inputmode="numeric" placeholder="2500"><div class="error" data-error="price"></div></div>
          <div class="field grow" style="margin:0"><label for="section">Section</label><input class="input" id="section" maxlength="40" list="secs" placeholder="Rice, Soups, Drinks"><datalist id="secs">${secs.map((x) => `<option value="${h(x)}">`).join('')}</datalist></div></div>
        <div class="field" style="margin:0"><label for="description">Description (optional)</label><input class="input" id="description" maxlength="240" placeholder="Smoky party jollof with fried chicken and plantain"></div>
        <div class="row" style="gap:8px;align-items:center"><label class="btn btn-sm btn-outline" style="width:auto;cursor:pointer">${icon('camera')} Photo<input type="file" accept="image/*" id="mpic" style="display:none"></label><span class="small muted grow" id="mpicname"></span></div>
        <button class="btn btn-primary" type="submit" id="msave">${icon('plus')} Add to menu</button>
        <div class="row" style="gap:8px;display:none" id="editbtns"><button class="btn btn-sm btn-outline grow" type="button" id="mcancel">Cancel</button><button class="btn btn-sm btn-outline grow" type="button" id="mdel" style="color:#D92D20">${icon('trash-can')} Delete</button></div>
      </form>
      <div class="small muted" style="line-height:1.5">Customers see only items switched on. Buja checks every price against this menu when an order comes in, so what they pay is what you listed.</div>
    </main>`;
  }, {
    async mount(el) {
      const f = el.querySelector('#mf'); if (!f) return;
      let editing = null, picId = null, menu = [];
      try { menu = (await api.myMenu()).menu; } catch {}
      el.querySelector('#dsave').addEventListener('click', async (e) => { const b = e.currentTarget; busy(b, true); try { await api.setDelivery({ deliveryFee: el.querySelector('#dfee').value, minOrder: el.querySelector('#dmin').value }); busy(b, false); toast('Delivery saved'); } catch (err) { busy(b, false); if (err.fields) showErrors(el, err.fields); failed(el, err); } });
      el.querySelector('#mpic').addEventListener('change', async (e) => { const file = e.target.files[0]; if (!file) return; const t = el.querySelector('#mpicname'); t.textContent = 'Uploading…'; try { picId = (await api.upload(await shrinkImg(file, 1000), 'image')).upload.id; t.textContent = 'Photo ready'; } catch (err) { t.textContent = ''; failed(el, err); } });
      const reset = () => { editing = null; picId = null; f.reset(); el.querySelector('#mftitle').textContent = 'Add an item'; el.querySelector('#msave').innerHTML = icon('plus') + ' Add to menu'; el.querySelector('#editbtns').style.display = 'none'; el.querySelector('#mpicname').textContent = ''; };
      el.querySelector('#mcancel').addEventListener('click', reset);
      el.querySelectorAll('[data-edit]').forEach((b) => b.addEventListener('click', () => {
        const m = menu.find((x) => String(x.id) === b.dataset.edit); if (!m) return; editing = m.id; picId = null;
        ['name', 'price', 'section', 'description'].forEach((k) => { f.querySelector('#' + k).value = m[k] ?? ''; });
        el.querySelector('#mftitle').textContent = 'Edit ' + m.name; el.querySelector('#msave').textContent = 'Save changes'; el.querySelector('#editbtns').style.display = '';
        el.querySelector('#mpicname').textContent = m.photo ? 'Has a photo. Choose another to replace it.' : '';
      ...
      f.addEventListener('submit', async (e) => {
        ...
        if (picId) body.uploadId = picId;
        try { if (editing) await api.updateMenuItem(editing, body); else await api.addMenuItem(body); toast(editing ? 'Saved' : 'Added'); location.reload(); }
        catch (err) { busy(b, false); if (err.fields) showErrors(el, err.fields); failed(el, err); }
      });
    }
  });
  ...
  route('/jobs/:id', { auth: true, tabs: '' }, async () => `<div class="bm-screen"><div class="bm-mapbox" id="map"></div>
    ...
    async mount(el, { id }) {
      ...
      const headline = (j) => {
        ...
        if (j.status === 'requested') return j.role === 'customer' ? `Waiting for ${n} to reply` : isOrder(j) ? 'New order' : 'New job request';
      ...
      const priceBlock = (j) => {
        if (j.items && j.items.length) return '';   // a menu order: the itemised total above is the agreed price
      ...
      /** A menu order: every line with its price, then delivery and the total both sides agreed. */
      ...
      const render = (j) => {
        ...
        if (C) {
          ...
          if (j.status === 'done' && j.other && j.other.id && j.role === 'customer') body += `<button class="btn btn-outline" data-save="${j.other.id}">${icon('bookmark')} ${isOrder(j) ? 'Save to my favourites' : 'Save to My mechanics'}</button><div class="small muted">They will be first to hear from you next time.</div>`;
          ...
          else if (j.status === 'requested' && j.items && j.items.length) body = `<div class="small muted">${j.km != null ? 'About ' + (j.km < 1 ? 'under 1' : j.km) + ' km from you. ' : ''}${j.landmark ? 'Landmark: ' + h(j.landmark) + '. ' : ''}You see the exact delivery spot once you accept.</div><div class="row" style="gap:8px"><button class="btn btn-primary grow" data-act="accept">Accept order</button><button class="btn btn-outline" data-act="decline" style="width:auto">Decline</button></div>`;
          else if (j.status === 'requested') body = `<div class="card" style="padding:12px;background:var(--surface);border:none"><div class="small muted">${isOrder(j) ? 'Order' : 'Problem'}</div><div style="font-size:15px;font-weight:600;margin-top:2px">${h(j.problem)}</div><div class="small muted" style="margin-top:6px">${j.km != null ? 'About ' + (j.km < 1 ? 'under 1' : j.km) + ' km from your workshop. ' : ''}You will see the exact spot once you accept.</div></div><div class="row" style="gap:8px"><button class="btn btn-primary grow" data-act="accept">Accept</button><button class="btn btn-outline" data-act="decline" style="width:auto">Decline</button></div>`;
          else if (j.status === 'accepted' && isOrder(j)) body = `${j.items && j.items.length ? (j.landmark ? `<div class="small">Landmark: ${h(j.landmark)}</div>` : '') : `<div class="card" style="padding:12px;background:var(--surface);border:none"><div class="small muted">Order</div><div style="font-size:15px;font-weight:600">${h(j.problem)}</div>${j.landmark ? `<div class="small" style="margin-top:4px">Landmark: ${h(j.landmark)}</div>` : ''}</div>`}
