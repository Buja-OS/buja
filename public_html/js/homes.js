...
export function registerHomes({ route, go, state, api, ui, DISTRICTS, failed }) {
  ...
  route('/homes/:id', { auth: true, tabs: '' }, async ({ id }) => {
    ...
    <div style="position:relative;height:300px;background:#2B3A31" id="gallery">${p.photos.map((x, i) => `<div data-slide="${i}" style="position:absolute;inset:0;${i ? 'display:none' : ''}">${ph(x.url)}</div>`).join('') || ph(null)}
      ...
      ${p.photos.length ? `<span style="position:absolute;right:14px;bottom:22px;background:rgba(0,0,0,.5);color:#fff;border-radius:12px;padding:4px 9px;font:600 12px Inter,system-ui;pointer-events:none;display:inline-flex;align-items:center;gap:5px;white-space:nowrap">${icon('camera')} ${p.photos.length} photo${p.photos.length === 1 ? '' : 's'}</span>` : ''}
    ...
    <main class="pad stack" style="gap:14px;padding-top:16px">
      ...
      ${p.landlord ? `<div class="card row" style="padding:14px;gap:12px">${p.landlord.photo ? `<img src="${p.landlord.photo}" alt="" data-zoom="${p.landlord.photo}" style="width:44px;height:44px;border-radius:22px;object-fit:cover;flex-shrink:0">` : avatar(p.landlord.name, 44, color(p.landlord.name))}<div class="grow"><div style="font-size:14px;font-weight:700" class="row" style="gap:6px">${h(p.landlord.name)}${p.landlord.verified ? ' ' + icon('circle-check') : ''}</div><div class="small muted">${window.bujaRating ? window.bujaRating(p.landlord.rating) + ' · ' : ''}${p.landlord.isCompany ? 'Real estate company' : 'Landlord'}${p.landlord.verified ? ' · title document checked' : ' · verification coming'} · ${p.landlord.listings} listing${p.landlord.listings === 1 ? '' : 's'}</div></div></div>` : ''}
      ...
      <div class="row" style="justify-content:space-between"><span class="small muted">${p.views} view${p.views === 1 ? '' : 's'}</span>${p.mine ? '' : `<a class="small" href="#/report/property/${p.id}" style="color:var(--ink-3);font-weight:600">${icon('triangle-exclamation')} Report</a>`}</div>
    </main>
  ...
  route('/homes/landlord/profile', { auth: true, tabs: '' }, async () => { const { landlord: l } = await api.landlordMe(); if (!l) { go('/homes/landlord'); return ''; } return `${topbar('Landlord profile', '/homes/landlord')}<form id="lf" class="pad stack" style="gap:14px">${field({ id: 'displayName', label: 'Name or company name', value: l.displayName })}<div class="field"><label>You are</label><div class="seg" id="co"><button type="button" data-v="0" class="${l.isCompany ? '' : 'on'}">A landlord</button><button type="button" data-v="1" class="${l.isCompany ? 'on' : ''}">A real estate company</button></div></div><div class="field"><label for="about">About</label><textarea class="input" id="about" maxlength="600" style="height:110px;padding:12px 14px;resize:none">${h(l.about || '')}</textarea></div><button class="btn btn-ink" type="submit">Save</button></form>`; },
    { mount(el) { if (!el.querySelector('#co')) return; el.querySelectorAll('#co button').forEach((b) => b.addEventListener('click', () => el.querySelectorAll('#co button').forEach((x) => x.classList.toggle('on', x === b)))); el.querySelector('#lf').addEventListener('submit', async (e) => { e.preventDefault(); const btn = e.target.querySelector('[type=submit]'); busy(btn, true); try { await api.landlordSave({ displayName: el.querySelector('#displayName').value, isCompany: el.querySelector('#co button.on').dataset.v === '1', about: el.querySelector('#about').value }); toast('Saved'); go('/homes/landlord'); } catch (err) { busy(btn, false); failed(el, err); } }); } });
