...
final class AccountController
{
    ...
    public function notifications(): void
    {
        ...
        foreach (['work', 'match', 'waka', 'offers', 'news', 'social', 'digest', 'learn'] as $k) if (array_key_exists($k, $b)) { $sets[] = "notify_$k = ?"; $p[] = !empty($b[$k]) ? 1 : 0; }
        if (!$sets) Http::json(['error' => 'validation', 'message' => 'Nothing to update.'], 422);
        $p[] = $u['id'];
        Db::run('UPDATE users SET ' . implode(', ', $sets) . ' WHERE id = ?', $p);
        $row = Db::one('SELECT * FROM users WHERE id = ?', [$u['id']]);
        Http::json(['notifications' => ['work' => (bool) $row['notify_work'], 'match' => (bool) $row['notify_match'], 'waka' => (bool) $row['notify_waka'], 'offers' => (bool) $row['notify_offers'], 'news' => (bool) $row['notify_news'], 'social' => (bool) $row['notify_social'], 'digest' => (bool) $row['notify_digest'], 'learn' => (bool) ($row['notify_learn'] ?? 1)]]);
    }
    ...
    public function today(): void
    {
        ...
        $n = Db::one('SELECT * FROM users WHERE id = ?', [$u['id']]);
