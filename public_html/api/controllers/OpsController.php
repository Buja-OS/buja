...
final class OpsController
{
    ...
    public function launch(): void
    {
        ...
        $m044 = $has('SELECT id FROM spot_claims LIMIT 1') && $has('SELECT id FROM job_payments LIMIT 1') && $has('SELECT owner_id FROM spots LIMIT 1') && $has('SELECT busy_until FROM artisans LIMIT 1') && $has('SELECT pay_mode FROM service_jobs LIMIT 1');
        ...
        $checks[] = ['id' => 'migrations', 'title' => 'Database up to date', 'ok' => $m036 && $m037 && $m038 && $m039 && $m040 && $m041 && $m042 && $m043 && $m044, 'weight' => 3,
            'detail' => ($m036 && $m037 && $m038 && $m039 && $m040 && $m041 && $m042 && $m043 && $m044) ? 'Migrations 036 to 044 are in (friends, escrow, kart shop, kart competition, order tracking, friend chats, kart stability, presence, statuses, menus and places, business claims and order payments).' : 'Missing: ' . implode(' and ', array_filter([$m036 ? '' : '036 (friends, saved mechanics, garage)', $m037 ? '' : '037 (escrow)', $m038 ? '' : '038 (kart shop)', $m039 ? '' : '039 (kart competition)', $m040 ? '' : '040 (order tracking)', $m041 ? '' : '041 (friend chats)', $m042 ? '' : '042 (kart stability)', $m043 ? '' : '043 (online status, statuses, menus, places)', $m044 ? '' : '044 (business claims, opening hours, paying for orders)'])) . '.',
