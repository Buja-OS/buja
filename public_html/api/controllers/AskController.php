...
final class AskController
{
    ...
    {
        $compact = array_map(fn($s) => ['id' => (int) $s['id'], 'name' => $s['name'], 'cat' => $s['category'], 'district' => $s['district'], 'tags' => json_decode($s['tags'] ?? '[]', true) ?: [], 'kind' => $s['subtype'] ?? null, 'price' => ($s['source'] ?? 'buja') === 'osm' ? null : (int) $s['price_level'], 'note' => $s['price_note'], 'desc' => mb_substr((string) $s['description'], 0, 140), 'hours' => $s['hours'], 'rating' => (Db::one('SELECT ROUND(AVG(stars),1) AS a, COUNT(*) AS n FROM spot_ratings WHERE spot_id = ?', [$s['id']]) ?: ['a' => null, 'n' => 0])], $spots);
