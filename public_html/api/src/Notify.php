...
final class Notify
{
    public static function user(int $userId, string $category, string $title, string $body, string $url, bool $emailFallback = false): void
    {
        try {
            ...
            foreach ($st->fetchAll() as $sub) {
                $code = WebPush::send($sub, ['title' => $title, 'body' => $body, 'url' => $url, 'tag' => $category], 'mailto:' . (string) Http::config('mail_from', 'hello@buja.ng'));
